...
public class ChunkProviderWasteland implements IChunkProvider
{
	...
	public void replaceBlocksForBiome(int xChunk, int zChunk, Block[] blocksForReplacement, byte[] metadataForReplacement, BiomeGenBase[] biomesForReplacement)
	{
		ChunkProviderEvent.ReplaceBiomeBlocks blockReplacementEvent = new ChunkProviderEvent.ReplaceBiomeBlocks(this, xChunk, zChunk, blocksForReplacement, biomesForReplacement);
		MinecraftForge.EVENT_BUS.post(blockReplacementEvent);
		if(blockReplacementEvent.getResult() == Result.DENY) return;
		
		this.stoneNoise = this.noiseGen4.func_151599_a(this.stoneNoise, (double)(xChunk * 16), (double)(zChunk * 16), 16, 16, 0.03125D * 2.0D, 0.03125D * 2.0D, 1.0D);
	...
	public Chunk provideChunk(int par1, int par2)
	{
		...
		this.replaceBlocksForBiome(par1, par2, blocks, metadata, this.biomesForGeneration);
	...
	private void func_147423_a(int par1, int par2, int par3)
	{
		...
		for(int i = 0; i < 5; ++i)
		{
			for(int j = 0; j < 5; ++j)
			{
				...
				++int1;
				double dbl12 = (double) flt1;
				double dbl14 = (double) flt0;
				dbl12 += dbl13 * 0.2D;
				...
				for(int k = 0; k < 33; ++k)
				{
					double dbl6 = ((double) k - dbl5) * 12.0D * 128.0D / 256.0D / dbl14;
	...
	public void populate(IChunkProvider par1IChunkProvider, int par2, int par3)
	{
		...
		MinecraftForge.EVENT_BUS.post(new PopulateChunkEvent.Pre(par1IChunkProvider, worldObj, rand, par2, par3, bln));
		...
		boolean doGen = TerrainGen.populate(par1IChunkProvider, worldObj, rand, par2, par3, bln, DUNGEON);
		...
		doGen = TerrainGen.populate(par1IChunkProvider, worldObj, rand, par2, par3, bln, ICE);
		...
		MinecraftForge.EVENT_BUS.post(new PopulateChunkEvent.Post(par1IChunkProvider, worldObj, rand, par2, par3, bln));
